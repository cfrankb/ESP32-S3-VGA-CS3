...
void VGA::attachPinToSignal(int pin, int signal)
{
	...
	auto err = gpio_set_drive_capability((gpio_num_t)pin, (gpio_drive_cap_t)3);
	if (err != ESP_OK)
	{
		ESP_LOGE(TAG, "gpio_set_drive_capability failed %x", err);
	}
}

bool VGA::init(const PinConfig pins, const Mode mode, int bits, int SCREEN)
{
	ESP_LOGI(TAG, "Init VGA");
	...
	{
		ESP_LOGE(TAG, "dmaBuffer is not valid");
		delete dmaBuffer;
		return false;
	}
	...
	auto err = gdma_new_channel(&dma_chan_config, &dmaCh);
	if (err != ESP_OK)
	{
		ESP_LOGE(TAG, "gdma_new_channel failed %x", err);
	}

	dmaChannel = (int)dmaCh;
	err = gdma_connect(dmaCh, GDMA_MAKE_TRIGGER(GDMA_TRIG_PERIPH_LCD, 0));
	if (err != ESP_OK)
	{
		ESP_LOGE(TAG, "gdma_new_channel failed %x", err);
	}
	...
	err = gdma_set_transfer_ability(dmaCh, &ability);
	if (err != ESP_OK)
	{
		ESP_LOGE(TAG, "gdma_new_channel failed %x", err);
	}
...
bool VGA::start()
{
	ESP_LOGI(TAG, "Starting VGA");
	auto err = gdma_reset((gdma_channel_handle_t)dmaChannel);
	if (err != ESP_OK)
	{
		ESP_LOGE(TAG, "gdma_reset failed %x", err);
	}
	esp_rom_delay_us(1);
	...
	LCD_CAM.lcd_user.lcd_update = 1;
	err = gdma_start((gdma_channel_handle_t)dmaChannel, (intptr_t)dmaBuffer->getDescriptor());
	if (err != ESP_OK)
	{
		ESP_LOGE(TAG, "gdma_start failed %x", err);
	}
	esp_rom_delay_us(1);
	LCD_CAM.lcd_user.lcd_update = 1;
...
{
	print(number, base, minCharacters);
	print("\n");
}
...
{
	n = n / 10;
	for (float x = 0; x < (a + a + 1); x = x + 1)
	{ // тут меняем шаг "x = x + 1", чем меньше шаг, тем выше четкость, но отрисовка дольше!!!
